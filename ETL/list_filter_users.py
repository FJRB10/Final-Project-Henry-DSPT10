...
def filtrar_csv_pandas(chunk_size=50000, **context):
    ...
    for chunk in pd.read_csv(local_path, chunksize=chunk_size):
        ...
        chunk_filtrado = chunk[chunk['user_id'].isin(list_business)]
...
with DAG(
    ...
    #sensor para esperar la tarea de filtrado de usuarios
    espera_ciudades = ExternalTaskSensor(
        task_id='espera_tarea_review',
        external_dag_id='filtra_review',
    ...
    download_task = PythonOperator(
        task_id=f'download',
        python_callable=download_file_user
    )
    # filtrar la lista por las ciudades
    filter_task = PythonOperator(
        task_id=f'filter_file',
        python_callable=filtrar_csv_pandas,
